...
export class CompteurImportComponent implements OnInit {
  ...
  chargerCompteur() {
    this.http.get<any>(`http://192.168.1.121:8080/api/compteur-import/${this.annee}`).subscribe({
      next: (data) => {
  ...
  modifierCompteur() {
    ...
    this.http.put<any>(`http://192.168.1.121:8080/api/compteur-import/${this.annee}`, null, { params }).subscribe({
      next: (data) => {
  ...
  initialiserCompteur() {
    ...
    this.http.post<any>(`http://192.168.1.121:8080/api/compteur-import/initialiser`, null, { params }).subscribe({
      next: (data) => {
