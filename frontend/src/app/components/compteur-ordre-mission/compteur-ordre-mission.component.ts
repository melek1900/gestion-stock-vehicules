...
export class CompteurOrdreMissionComponent implements OnInit {
  ...
  chargerCompteur() {
    this.http.get<any>(`http://172.20.10.8:8080/api/compteur-ordre-mission/${this.annee}`).subscribe({
      next: (data) => {
  ...
  modifierCompteur() {
    ...
    this.http.put<any>(`http://172.20.10.8:8080/api/compteur-ordre-mission/${this.annee}`, null, { params }).subscribe({
      next: (data) => {
  ...
  initialiserCompteur() {
    ...
    this.http.post<any>(`http://172.20.10.8:8080/api/compteur-ordre-mission/initialiser`, null, { params }).subscribe({
      next: (data) => {
