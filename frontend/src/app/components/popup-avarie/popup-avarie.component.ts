...
export class PopupAvarieComponent {
  ...
  sauvegarder() {
    ...
    this.http.put(`http://172.20.10.8:8080/api/vehicules/${this.data.vehicule.numeroChassis}/avaries`, formData, { headers }).subscribe({
      next: () => {
  ...
  reparerVehicule() {
    ...
    this.http.patch(
      `http://172.20.10.8:8080/api/vehicules/${this.data.vehicule.numeroChassis}/reparer`,
      body,
      { headers }
    ).subscribe({
      next: (res) => {
