...
export class VeiculeTransportComponent {
  ...
  loadVehicules() {
    this.http.get<any[]>('http://172.20.10.8:8080/api/vehicules-transport').subscribe({
      next: (data) => {
        this.vehicules = data;
  ...
  openAddDialog() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.http.post('http://172.20.10.8:8080/api/vehicules-transport', result).subscribe({
          next: () => {
  ...
  editVehicule(vehicule: any) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.http.put(`http://172.20.10.8:8080/api/vehicules-transport/${vehicule.id}`, result).subscribe({
          next: () => {
  ...
  deleteVehicule(id: number) {
    if (confirm('Voulez-vous vraiment supprimer ce véhicule ?')) {
      this.http.delete(`http://172.20.10.8:8080/api/vehicules-transport/${id}`).subscribe({
